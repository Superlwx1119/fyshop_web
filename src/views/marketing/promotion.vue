      ...
      >
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="primary"
            @click="goDetail('edit', scope.row.promotionId)"
          >
            编辑
          </el-button>
          <el-button
          ...
          <el-button
            size="mini"
            v-if="scope.row.stateValue != '已结束'"
            type="danger"
            @click="overSubmit(scope.row)"
          >
            下架
          </el-button>
          <el-button
...
import { getpromotionActivityList, promotionActivityAlterStatus , saveGroupActivity, getPromotionActivityResultsData, promotionActivityAlterState } from '@/api/marketing'
...
export default class extends Vue {
  ...
  private async getPromotionActivityResultsData(id) {
    const { data } = await getPromotionActivityResultsData({
      data: { promotionId: id }
    })
  ...
  private async getList() {
    ...
    const { data } = await getpromotionActivityList({
      data: this.listQuery,
  ...
  private openDialog(status: string, index: any, row: any) {
    ...
    this.getPromotionActivityResultsData(row.promotionId)
  }
  ...
  private async delAssemble(id: string) {
    const { data } = await promotionActivityAlterState({
      data: {
        promotionId: id,
  ...
  private  promotionActivityAlterStatus(id:any){
    
      promotionActivityAlterStatus({data: {promotionId: id}}).then(res=>{
          this.$message({
            message: res.data.resultInfo,
            type: res.data.resultCode == 1 ? 'success' : 'warning'
          })
          this.getList()
        }) 
  ...
  private overSubmit(group: any){
    // if (group.stateValue == '进行中') {
    //   this.$message.error('活动进行中无法下架!')
    //   return
    // }
    this.$confirm(`确认下架活动?`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
      .then(() => {
        this.promotionActivityAlterStatus(group.promotionId)
      })
      .catch(error => {
        this.$message({
          type: 'info',
  ...
  private deleteSubmit(group: any) {
    ...
    this.$confirm(`确认删除活动?`, '提示', {
      ...
      .then(() => {
        this.delAssemble(group.promotionId)
      })
      .catch(error => {
        this.$message({
          type: 'info',
