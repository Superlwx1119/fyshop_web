...
import { promotionActivitySave, getpromotionActivityDetail } from '@/api/marketing'
...
export default class extends Vue {
  ...
  created() {
    ...
    this.queryInfo.status === 'edit' && this.getpromotionActivityDetail()
  }
  ...
  private async postDetail(postData: any) {
    const { data } = await promotionActivitySave({
      data: postData
    })
    this.$message({
  ...
  private batchSettingPriceAndstock() {
    ...
    for (const item of tableData) {
      item.promotionPrice = this.postDetailList.price
  ...
  private editGoods = []
  private async getpromotionActivityDetail() {
    this.listLoading = true
    const { data } = await getpromotionActivityDetail({
      data: {
        promotionId: this.queryInfo.id
      }
    })
  ...
  private setPrice() {
    ...
    // console.log("促销商品展示", this.postList);
      ...
      let groupGoods = [
        {
          ...
          promotionPrice: this.postDetailList.price,
    ...
    (this.$refs[postList] as ElForm).validate((valid: boolean) => {
      if (valid) {
        let postData = {
          ...
          promotionId: this.postList.groupId ? this.postList.groupId : '',
