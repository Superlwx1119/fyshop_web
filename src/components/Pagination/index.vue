...
export default class extends Vue {
  ...
  @Prop({ default: 20 }) private currentPages!: number;
  ...
  handleSizeChange(value: number) {
    this.$emit("pagination", { page: this.currentPages, currentPage: value });
    if (this.autoScroll) {
      scrollTo(0, 800);
    }
  }
  ...
  handleCurrentChange(value: number) {
    this.$emit("pagination", { page: value, currentPage: this.pageSize });
    if (this.autoScroll) {
      scrollTo(0, 800);
    }
  }
